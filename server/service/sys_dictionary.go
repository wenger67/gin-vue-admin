...
func CreateSysDictionary(sysDictionary model.SysDictionary) (err error) {
	if (!errors.Is(global.PantaDb.First(&model.SysDictionary{}, "type = ?", sysDictionary.Type).Error, gorm.ErrRecordNotFound)) {
		return errors.New("存在相同的type，不允许创建")
	}
	err = global.PantaDb.Create(&sysDictionary).Error
	return err
}
...
func DeleteSysDictionary(sysDictionary model.SysDictionary) (err error) {
	err = global.PantaDb.Delete(sysDictionary).Delete(&sysDictionary.SysDictionaryDetails).Error
	return err
}
...
func UpdateSysDictionary(sysDictionary *model.SysDictionary) (err error) {
	...
	db := global.PantaDb.Where("id = ?", sysDictionary.ID).First(&dict)
...
func GetSysDictionary(Type string, Id uint) (err error, sysDictionary model.SysDictionary) {
	err = global.PantaDb.Where("type = ? OR id = ?", Type, Id).Preload("SysDictionaryDetails").First(&sysDictionary).Error
	return
}
...
func GetSysDictionaryInfoList(info request.SysDictionarySearch) (err error, list interface{}, total int64) {
	...
	// 创建db
	db := global.PantaDb.Model(&model.SysDictionary{})
